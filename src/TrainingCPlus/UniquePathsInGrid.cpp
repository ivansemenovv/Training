#include "stdafx.h"
#include "UniquePathsInGrid.h"

using namespace std;

//"Given a grid of size m * n, lets assume you are starting at (1,1) and your goal is to reach (m,n). 
// At any instance, if you are on (x,y), you can either go to (x, y + 1) or (x + 1, y).
//
//Now consider if some obstacles are added to the grids.How many unique paths would there be ?
//An obstacle and empty space is marked as 1 and 0 respectively in the grid.
//
//Example :
//    There is one obstacle in the middle of a 3x3 grid as illustrated below.
//
//    [
//        [0, 0, 0],
//        [0, 1, 0],
//        [0, 0, 0]
//    ]
//The total number of unique paths is 2."

void UniquePathsInGrid::RunTestCases()
{
    vector<vector<int>> A{
        vector<int> {0,0,0,0},
        vector<int> {0,1,0,0},
        vector<int> {0,0,0,0}
    };

    TestHelper::IsEquals(4, GetUniquePathsCount(A));
}

int UniquePathsInGrid::GetUniquePathsCount(vector<vector<int>>& A)
{
    // init dp matrix and fill it with 0
    vector<vector<int>> dp(A.size(), vector<int>(A[0].size(), 0));

    // fill last column with 1
    for (int i = dp.size() - 2; i >= 0; i--)
    {
        if (A[i][A[0].size() - 1] == 0)
            dp[i][dp[0].size() - 1] = 1;
        else
            break;
    }

    // fill last row with 1
    for (int j = dp[dp.size() - 1].size() - 2; j >= 0; j--)
    {
        if (A[dp.size() - 1][j] == 0)
            dp[dp.size() - 1][j] = 1;
        else
            break;
    }

    for (int i = dp.size() - 2; i >= 0; i--)
    {
        for (int j = dp[i].size() - 2; j >= 0; j--)
        {
            if (A[i][j] == 0)
            {
                dp[i][j] = dp[i + 1][j] + dp[i][j + 1];
            }
        }
    }

    return dp[0][0];
}
